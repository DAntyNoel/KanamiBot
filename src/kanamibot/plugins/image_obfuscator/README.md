...
私聊中精确发送 `混淆` 或 `解混淆` 后，插件会等待 60 秒；每收到一条图片消息都会重新计时并继续等待。发送 `结束`、启动另一个混淆/解混淆命令或超时会结束本轮交互，并发送结束提醒。收到的消息记录、引用消息和当前消息中的多张图片都会处理。混淆保存输入图，解混淆保存解码后的图，位置为 `data/image_obfuscator/source/{encode,decode}/`。

普通群聊允许直接附图触发混淆，但不启动后续交互式发图；不允许触发解混淆，群聊触发解混淆时请改为私聊机器人。好友、陌生人私聊以及群聊临时会话允许两种操作。
